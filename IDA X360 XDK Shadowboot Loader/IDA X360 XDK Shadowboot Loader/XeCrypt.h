...
//bool XeCryptBnQwBeSigCreate2(u64* output, const u8* hash, const u8* salt, const XECRYPT_RSA* key);
...
// returns:	TRUE if successful
//			FALSE if error
//bool XeCryptBnQwBeSigVerify2(u64* sig, const u8* hash, const u8* salt, const XECRYPT_RSA* key);
...
// returns:	TRUE if successful
//			FALSE if error
//bool XeCryptBnQwNeRsaPrvCrypt2(const u64* input, u64* output, const XECRYPT_RSA* key); // XeRsaKey
